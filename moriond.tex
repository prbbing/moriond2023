...
as seen in Figure (~\ref{fig:tttt}~\subref{fig:mass}). A recent ATLAS analysis
...
parameters, as seen in Figure (~\ref{fig:tttt}~\subref{fig:ttttlimits}). The
...
peaks, as illustrated in Figure (~\ref{fig:periodic}~\subref{fig:signal}). The
...
(~\ref{fig:periodic}~\subref{fig:shape}). Limits are set for the gravity scale,
$M_{5}$, and the turn-on mass scale, $k$, shown in Figure
(~\ref{fig:periodic}~\subref{fig:perlimits})~\cite{period}. This search not
